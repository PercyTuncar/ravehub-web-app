...
export function EnhancedStickyCTA({
  ...
  const getZoneAvailability = (zoneId: string) => {
    ...
    const percentage =
      zone.capacity > 0 ? Math.round((available / zone.capacity) * 100) : 0;
          ...
          <Card className="border-0 shadow-none rounded-none md:rounded-2xl">
            <CardContent className="p-0">
              ...
              <div className="p-4">
                ...
                {/* Main CTA Button */}
                <Link
                  href={`/eventos/${event.slug}/entradas${selectedZone ? `?zone=${selectedZone}` : ""}`}
                  className="block"
                >
                  <Button
                    size="lg"
                          ...
                          .map((zp) => {
                            ...
                            return (
                              <motion.button
                                key={zp.zoneId}
                                onClick={() =>
                                  setSelectedZone(isSelected ? null : zp.zoneId)
                                }
                                disabled={isSoldOut}
                                className={cn(
                                ...
                                {!isSoldOut && (
                                  <div className="space-y-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {availability.available} disponibles de{" "}
