  ...
  const eventSchema = {
    ...
    performer:
      event.artistLineupIds && event.artistLineupIds.length > 0
        ? event.artistLineupIds.map((artistId) => ({
    ...
    offers: {
      ...
      highPrice:
        offers.length > 0 ? Math.max(...offers.map((o) => o.price)) : 0,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: event.name,
        item: `https://www.ravehublatam.com/eventos/${event.slug}`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: "Entradas",
        item: `https://www.ravehublatam.com/eventos/${event.slug}/entradas`,
      },
    ],
  };
