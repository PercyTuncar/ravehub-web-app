...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return generateMetadataForCountry('PE', events) || {
    ...
    description: 'Descubre eventos de música electrónica en Perú'
  }
}

export default async function PeruPage() {
  const events = await getPeruEvents()
  const schemaGenerator = new CountrySchemaGenerator('PE')
  const jsonLd = schemaGenerator.generateCountryPageSchema(events)
  const seoContent = generateSEOContent('PE', events)
  ...
  return (
    <main className="min-h-screen bg-black">
            ...
            <div className="row-span-2 flex items-center justify-center">
              <div className="text-center space-y-4">
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold leading-tight">
                  ...
                  <div className="text-red-500 text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold">Perú</div>
                </h1>
              </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-16">
                <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-red-400 to-red-600 bg-clip-text text-transparent">
                  Música Electrónica en Todo Perú
                </h2>
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="text-center group">
                  <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 rounded-3xl p-8 border border-red-500/20 hover:border-red-500/40 transition-all duration-500 hover:scale-105">
                    <div className="text-4xl md:text-5xl font-bold text-red-400 mb-3">{upcomingEvents.length}</div>
                    <div className="text-gray-200 text-lg font-medium">Eventos Próximos</div>
                    <div className="text-sm text-gray-400 mt-2">En todo Perú</div>
                  </div>
                </div>
                <div className="text-center group">
                  <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 rounded-3xl p-8 border border-red-500/20 hover:border-red-500/40 transition-all duration-500 hover:scale-105">
                    ...
                    <div className="text-sm text-gray-400 mt-2">Lima, Cusco y más</div>
                  </div>
                </div>
                <div className="text-center group">
                  <div className="bg-gradient-to-br from-red-500/20 to-red-600/10 rounded-3xl p-8 border border-red-500/20 hover:border-red-500/40 transition-all duration-500 hover:scale-105">
          ...
          {upcomingEvents.length === 0 ? (
            <div className="text-center py-16">
              <div className="bg-gray-900/50 rounded-3xl p-12 border border-gray-700/50">
                ...
                <p className="text-gray-500">Mantente atento a las novedades de música electrónica en Perú</p>
              </div>
            </div>
          ) : (
            <>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {upcomingEvents.slice(0, 9).map((event) => (
                  <Card key={event.id} className="group overflow-hidden bg-gray-900/50 border-gray-700/50 hover:border-red-500/50 transition-all duration-500">
                    <div className="relative h-48 overflow-hidden">
                      <Image
                        src={event.mainImageUrl || '/placeholder-event.jpg'}
                        alt={`${event.name} - Evento de música electrónica en ${event.location.city}, Perú`}
                        fill
                    ...
                    <CardContent className="p-6">
                      ...
                      <Link href={`/eventos/${event.slug}`} className="block">
                        <Button className="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-semibold rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg">
              ...
              {upcomingEvents.length > 9 && (
                <div className="text-center mt-16">
                  <Link href="/eventos?country=PE">
                    <Button variant="outline" size="lg" className="border-red-500/50 text-red-400 hover:bg-red-500/10">
                      {seoContent?.ctaText}
      ...
      <CountryAbout data={COUNTRY_ABOUT.PE} />

      <CountryFAQ countryName="Perú" faqs={COUNTRY_FAQS.PE} />
    </main>
  )
}
