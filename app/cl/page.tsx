    ...
    <main className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-blue-400 to-blue-300 bg-clip-text text-transparent">
            Eventos en Chile
          </h1>
          <p className="text-xl md:text-2xl mb-8 text-gray-200">
            La escena electrónica más diversa y vibrante del sur
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/eventos" className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Ver todos los eventos en Chile">
              Ver todos los eventos
            </a>
            <a href="#eventos" className="border-2 border-white text-white hover:bg-white hover:text-blue-900 font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Explorar eventos destacados">
              Explorar destacados
            </a>
          </div>
        </div>
      ...
      <section id="eventos" className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-4">Próximos eventos en Chile</h2>
          <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
            Desde Santiago hasta Punta Arenas, descubre los mejores eventos de música electrónica de Chile.
          </p>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-8">
            Explora eventos por ciudad en Chile
          </p>

          <div className="flex flex-wrap justify-center gap-4" role="list" aria-label="Ciudades principales de Chile">
            {[
              { name: 'Santiago', count: events.filter(e => e.location.city?.toLowerCase().includes('santiago')).length },
              { name: 'Valparaíso', count: events.filter(e => e.location.city?.toLowerCase().includes('valparaíso')).length },
              { name: 'Viña del Mar', count: events.filter(e => e.location.city?.toLowerCase().includes('viña')).length },
              { name: 'Concepción', count: events.filter(e => e.location.city?.toLowerCase().includes('concepción')).length },
              { name: 'La Serena', count: events.filter(e => e.location.city?.toLowerCase().includes('serena')).length },
              { name: 'Antofagasta', count: events.filter(e => e.location.city?.toLowerCase().includes('antofagasta')).length }
            ].map((city) => (
              <a
                key={city.name}
                href={`/eventos?region=${city.name.toLowerCase()}`}
                className="px-6 py-3 bg-blue-50 hover:bg-blue-100 text-blue-800 hover:text-blue-900 dark:bg-blue-900/20 dark:hover:bg-blue-800/30 dark:text-blue-200 dark:hover:text-blue-100 rounded-full transition-colors"
      ...
      <section className="py-16 px-4 bg-blue-900 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">No te pierdas el próximo rave en Chile</h2>
          <p className="text-blue-100 mb-8">
            Recibe preventas exclusivas y lineups de los mejores eventos en Chile.
          </p>

          <form className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto" aria-label="Suscripción al newsletter de Chile">
            <input
              type="email"
