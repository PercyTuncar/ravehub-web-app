  ...
  "@graph": [
    {
      ...
      "mainEntity": {
        "@type": "ItemList",
        "name": "Eventos de música electrónica en Colombia",
        "description": "Lista completa de eventos de música electrónica disponibles en Colombia"
      },
      "inLanguage": "es-419",
...
export default async function ColombiaPage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-green-900 via-green-800 to-green-900 text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl md:text-2xl mb-8 text-gray-200">
            La capital mundial de la música electrónica
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/eventos" className="bg-green-500 hover:bg-green-600 text-white font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Ver todos los eventos en Colombia">
              Ver todos los eventos
            </a>
            <a href="#eventos" className="border-2 border-white text-white hover:bg-white hover:text-green-900 font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Explorar eventos destacados">
              Explorar destacados
            </a>
          </div>
        </div>
      ...
      <section id="eventos" className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-4">Próximos eventos en Colombia</h2>
          <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
            Desde Bogotá hasta Cartagena, descubre los mejores eventos de música electrónica de Colombia.
          </p>
            ...
            <div className="text-center mt-12">
              <Link href="/eventos">
                <Button variant="outline" size="lg">
                  Ver todos los eventos en Colombia
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </Link>
            </div>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-8">
            Explora eventos por ciudad en Colombia
          </p>

          <div className="flex flex-wrap justify-center gap-4" role="list" aria-label="Ciudades principales de Colombia">
            {[
              { name: 'Bogotá', count: events.filter(e => e.location.city?.toLowerCase().includes('bogotá')).length },
              { name: 'Medellín', count: events.filter(e => e.location.city?.toLowerCase().includes('medellín')).length },
              { name: 'Cali', count: events.filter(e => e.location.city?.toLowerCase().includes('cali')).length },
              { name: 'Barranquilla', count: events.filter(e => e.location.city?.toLowerCase().includes('barranquilla')).length },
              { name: 'Cartagena', count: events.filter(e => e.location.city?.toLowerCase().includes('cartagena')).length },
              { name: 'Pereira', count: events.filter(e => e.location.city?.toLowerCase().includes('pereira')).length },
              { name: 'Bucaramanga', count: events.filter(e => e.location.city?.toLowerCase().includes('bucaramanga')).length }
            ].map((city) => (
              <a
                key={city.name}
                href={`/eventos?region=${city.name.toLowerCase()}`}
                className="px-6 py-3 bg-green-50 hover:bg-green-100 text-green-800 hover:text-green-900 dark:bg-green-900/20 dark:hover:bg-green-800/30 dark:text-green-200 dark:hover:text-green-100 rounded-full transition-colors"
      ...
      <section className="py-16 px-4 bg-green-900 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">No te pierdas el próximo rave en Colombia</h2>
          <p className="text-green-100 mb-8">
            Recibe preventas exclusivas y lineups de los mejores eventos en Colombia.
          </p>

          <form className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto" aria-label="Suscripción al newsletter de Colombia">
            <input
              type="email"
