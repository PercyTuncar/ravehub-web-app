  ...
  "@graph": [
    {
      ...
      "mainEntity": {
        "@type": "ItemList",
        "name": "Eventos de música electrónica en Argentina",
        "description": "Lista completa de eventos de música electrónica disponibles en Argentina"
      },
      "inLanguage": "es-419",
...
async function getArgentinaEvents(): Promise<Event[]> {
  try {
    const { events } = await getEventsByCountry('AR')
    return events
  } catch (error) {
    console.error('Error loading Argentina events:', error)
    return []
  }
}

export default async function ArgentinaPage() {
  const events = await getArgentinaEvents()

  return (
    <main className="min-h-screen">
      <JsonLd id="argentina-page-jsonld" data={jsonLd} />

      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-cyan-900 via-cyan-800 to-cyan-900 text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-cyan-400 to-cyan-300 bg-clip-text text-transparent">
            Eventos en Argentina
          </h1>
          <p className="text-xl md:text-2xl mb-8 text-gray-200">
            La pasión electrónica del Cono Sur
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/eventos" className="bg-cyan-500 hover:bg-cyan-600 text-white font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Ver todos los eventos en Argentina">
              Ver todos los eventos
            </a>
            <a href="#eventos" className="border-2 border-white text-white hover:bg-white hover:text-cyan-900 font-semibold py-3 px-8 rounded-lg transition-colors text-center" aria-label="Explorar eventos destacados">
              Explorar destacados
            </a>
          </div>
        </div>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div>
              <div className="text-3xl font-bold text-cyan-600 mb-2">{events.length}</div>
              <div className="text-gray-600">Eventos activos</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-cyan-600 mb-2">9</div>
      ...
      <section id="eventos" className="py-16 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-4">Próximos eventos en Argentina</h2>
          <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
            Desde Buenos Aires hasta Mendoza, descubre los mejores eventos de música electrónica de Argentina.
          </p>
            ...
            <div className="text-center mt-12">
              <Link href="/eventos">
                <Button variant="outline" size="lg">
                  Ver todos los eventos en Argentina
                  <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </Link>
            </div>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-gray-600 mb-8">
            Explora eventos por ciudad en Argentina
          </p>

          <div className="flex flex-wrap justify-center gap-4" role="list" aria-label="Ciudades principales de Argentina">
            {[
              { name: 'Buenos Aires', count: events.filter(e => e.location.city?.toLowerCase().includes('buenos aires')).length },
              { name: 'Córdoba', count: events.filter(e => e.location.city?.toLowerCase().includes('córdoba')).length },
              { name: 'Rosario', count: events.filter(e => e.location.city?.toLowerCase().includes('rosario')).length },
              { name: 'Mendoza', count: events.filter(e => e.location.city?.toLowerCase().includes('mendoza')).length },
              { name: 'La Plata', count: events.filter(e => e.location.city?.toLowerCase().includes('la plata')).length },
              { name: 'Mar del Plata', count: events.filter(e => e.location.city?.toLowerCase().includes('mar del plata')).length },
              { name: 'Salta', count: events.filter(e => e.location.city?.toLowerCase().includes('salta')).length },
              { name: 'Santa Fe', count: events.filter(e => e.location.city?.toLowerCase().includes('santa fe')).length }
            ].map((city) => (
              <a
                key={city.name}
                href={`/eventos?region=${city.name.toLowerCase()}`}
                className="px-6 py-3 bg-cyan-50 hover:bg-cyan-100 text-cyan-800 hover:text-cyan-900 dark:bg-cyan-900/20 dark:hover:bg-cyan-800/30 dark:text-cyan-200 dark:hover:text-cyan-100 rounded-full transition-colors"
      ...
      <section className="py-16 px-4 bg-cyan-900 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">No te pierdas el próximo rave en Argentina</h2>
          <p className="text-cyan-100 mb-8">
            Recibe preventas exclusivas y lineups de los mejores eventos en Argentina.
          </p>

          <form className="flex flex-col sm:flex-row gap-4 justify-center max-w-md mx-auto" aria-label="Suscripción al newsletter de Argentina">
            <input
              type="email"
